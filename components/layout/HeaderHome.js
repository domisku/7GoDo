    ...
    <nav className="flex justify-around top-0 items-center h-20 fixed w-full bg-white z-20">
      ...
      {!session && (
        <ul className="flex h-20">
          <Link href="/signin">
            <li onClick={() => setLoading(true)} className="flex items-center h-full px-2 sm:px-6 sm:text-xl cursor-pointer border-b-2 border-transparent hover:border-red-500 hover:bg-red-50">
              Log In
            </li>
          </Link>
        </ul>
